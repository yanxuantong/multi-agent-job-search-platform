...
def tailor_resume(state: JobSearchState) -> NodeResult:
    ...
    story_context = retrieve_story_context(
        state.story_bank,
        jd.required_skills,
        query=f"resume proof points for {jd.role_title} at {jd.company_name}",
    )
    ...
    state.messages.append(f"Resume context assembled from {story_context.returned_count} retrieved story chunks.")
