...
CHAPTERS: list[Chapter] = [
    ...
    Chapter(
        ...
        [
            "本项目的 local RAG 是教学入口：先用本地、可解释、低成本方式理解 query、match、rank、context assembly。",
        ...
        [
            "用 local RAG 对 story bank 做关键词检索，观察返回内容是否真的适合面试准备。",
            "为检索结果加上 source/title 字段，思考如何在 UI 里显示出处。",
            "列出何时 local keyword retrieval 不够，需要 embedding 或 reranker。",
        ],
        [
            "RAG 的首个 eval 应该测 retrieval 还是 final answer？两者都要，但先测 retrieval 命中更可控。",
            "是否需要 pgvector？当数据量、语义匹配需求、跨文档检索需求上来后再引入。",
            "如何避免过期信息？source timestamp、refresh policy、行业动态单独标注。",
        ],
        [
            ...
            "这和 JobAgent 的学习路径一致：先把本地检索做可见，再逐步升级。",
        ],
        [
